...
public class RUBiSProperties
{
   ...
   * rubis.properties file is not found in the classpath, the current thread is
   * killed.
   */
  public RUBiSProperties()
  {
    // Get and check database.properties
    System.out.println("Looking for rubis.properties in classpath ("
    ...
    {
      System.err
          .println("No rubis.properties file has been found in your classpath.<p>");
  ...
  /**
   * Creates a new <code>RUBiSProperties</code> instance. If the
   * filename.properties file is not found in the classpath, the current thread
   * is killed.
   * 
  ...
  {
    // Get and check database.properties
    System.out.println("Looking for " + filename + ".properties in classpath ("
        + System.getProperty("java.class.path", ".") + ")<p>");
    try
    {
    ...
    catch (java.util.MissingResourceException e)
    {
      System.err.println("No " + filename
          + ".properties file has been found in your classpath.<p>");
      Runtime.getRuntime().exit(1);
    }
  }
  ...
  /**
   * Check for all needed fields in rubis.properties and inialize corresponding
   * values. This function returns the corresponding URLGenerator on success.
   * 
  ...
  public URLGenerator checkPropertiesFileAndGetURLGenerator()
  {
    try
    {
      ...
      useTPCWThinkTime = getProperty("workload_use_tpcw_think_time").compareTo(
          "yes") == 0;
      if (useTPCWThinkTime)
      ...
      BufferedReader regionsReader = new BufferedReader(new FileReader(
          getProperty("database_regions_file")));
      ...
      BufferedReader categoriesReader = new BufferedReader(new FileReader(
          getProperty("database_categories_file")));
      ...
      while (categoriesReader.ready())
      {
        ...
        {
          System.err.println("Syntax error in categories file on line "
              + nbOfCategories + ": " + line);
          return null;
        }
        Integer nbOfItems = new Integer(line.substring(openParenthesis + 1,
            closeParenthesis));
      ...
      for (int i = 0; i < nbOfCategories; i++)
        itemsPerCategory[i] = ((Integer) itemsPerCategoryV.elementAt(i))
            .intValue();
      System.out.println(nbOfCategories + " categories found.<br>");
      System.out.println("Total number of items to generate: "
          + totalActiveItems + "<br>");
      ...
      percentUniqueItems = new Float(
          getProperty("database_percentage_of_unique_items"));
      System.out.println(percentUniqueItems + "%" + "<br>");
      System.out.print("Percentage of items with reserve price : ");
      percentReservePrice = new Float(
          getProperty("database_percentage_of_items_with_reserve_price"));
      System.out.println(percentReservePrice + "%" + "<br>");
      System.out.print("Percentage of buy now items            : ");
      percentBuyNow = new Float(
          getProperty("database_percentage_of_buy_now_items"));
      System.out.println(percentBuyNow + "%" + "<br>");
      System.out.print("Maximum quantity for multiple items    : ");
      maxItemQty = new Integer(
          getProperty("database_max_quantity_for_multiple_items"));
      System.out.println(maxItemQty + "<br>");
      System.out.print("Item description maximum lenth         : ");
      itemDescriptionLength = new Integer(
          getProperty("database_item_description_length"));
      ...
      maxCommentsPerUser = new Integer(
          getProperty("database_max_comments_per_user"));
  ...
  /**
   * Get the total number of regions found in the region file given in the
   * regions_file field
   * 
  ...
  /**
   * Get the total number of categories found in the categories file given in
   * the categories_file field
   * 
  ...
  /**
   * Get a vector of region names as found in the region file given in the
   * regions_file field
   * 
  ...
  /**
   * Get a vector of category names as found in the categories file given in the
   * categories_file field
   * 
  ...
  /**
   * Return an array of number of items per category as described in the
  ...
  /**
   * Get the total number of old items (auction date is over) to be inserted in
   * the database.
   * 
  ...
  /**
   * Get the percentage of unique items given in the percentage_of_unique_items
   * field
   * 
  ...
  /**
   * Get the percentage of items with a reserve price given in the
   * percentage_of_items_with_reserve_price field
   * 
  ...
  /**
   * Get the percentage of items that users can 'buy now' given in the
   * percentage_of_buy_now_items field
   * 
  ...
  /**
   * Get the maximum quantity for multiple items given in the
   * max_quantity_for_multiple_items field
   * 
  ...
  /**
   * Get the maximum item description length given in the
   * item_description_length field
   * 
  ...
  /**
   * Get the maximum number of bids per item given in the max_bids_per_item
   * field
   * 
  ...
  /**
   * @deprecated Comments are now generated per item and no more per user, so
   *             this function should not be used anymore. Get the maximum
   *             number of comments per user given in the max_comments_per_user
   *             field
  ...
  /**
   * Get the number of items to display per page (when browsing) given in the
   * number_of_items_per_page field
   * 
