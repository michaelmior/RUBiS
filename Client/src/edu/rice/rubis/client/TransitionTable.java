...
/**
 * This class provides support for transitions between RUBiS web site pages. A
 ...
 * <code>probability_x_y</code> determines the probability to go from state x
 * to state y. There are 2 extra lines compared to colums. These lines are:
 * <code>Back probability:</code> probability to go back to last page (like
 * the back button of the browser) <code>End of session:</code> probability
 * that the user ends the session (leave the web site).
 * 
 * </pre>
 * 
...
public class TransitionTable
{
   ...
   */
  public TransitionTable(int columns, int rows, Stats statistics,
      boolean UseTPCWThinkTime)
  {
    nbColumns = columns;
    nbRows = rows;
    stats = statistics;
  ...
  public int nextState()
  {
        ...
        try
        {
          if (useTPCWThinkTime)
            Thread.currentThread().sleep(
                (long) ((float) TPCWthinkTime() * ClientEmulator
                    .getSlowDownFactor()));
          else
            Thread.currentThread().sleep(
                (long) ((float) transitionsTime[currentState] * ClientEmulator
                    .getSlowDownFactor()));
        }
    ...
    { // Add this state to history (previousStates) if needed
      if (!isEndOfSession())
      { // If there is no probability to go back from this state, just empty the
        // stack
        if (transitions[currentState][nbRows - 2] == 0)
          previousStates.removeAllElements();
        else
          // else add the previous state to the history just in case we go back
          // !
  ...
  /**
   * Display the transition matrix on the standard output. This function is only
  ...
  protected void displayMatrix()
  {
    ...
    System.out
        .println("<TABLE border=\"1\" summary=\"transition table\"><TBODY>\n");
    ...
    for (i = 0; i < nbRows; i++)
    {
      System.out.print("\n<TR><TD><div align=left><B>" + stateNames[i]
          + "</B></div>");
      for (j = 0; j < nbColumns; j++)
        System.out.print("<TD><div align=right>"
            + Float.toString(transitions[j][i]) + "</div>");
      System.out.print("<TD><div align=right>"
          + Float.toString(transitionsTime[i]) + "</div>");
    }
    System.out.println("\n</TBODY></TABLE>\n");
    System.out.println();
  }
